...
describe("WIRY token", function () {
  ...
  describe("Mint", () => {
    it("Should mint tokens to users", async () => {
      ...
      expect(clientEndBalance.sub(clientStartBalance)).to.equal(mintAmount);
    });
  ...
  describe("Burn", () => {
    ...
    it("Should fail to burn tokens if caller is not the owner", async () => {
      ...
      await expect(
        wiry.connect(clientAcc1).burnFrom(clientAcc1.address, burnAmount)
      ).to.be.revertedWith("Ownable: caller is not the owner");
    });
  });
  ...
  describe("Blacklist", () => {
    describe("Add to balcklist", () => {
      it("Should add users to the blacklist", async () => {
        ...
        expect(await wiry.blackListed(clientAcc1.address)).to.equal(false);

        await wiry.addToBlackList(clientAcc1.address);

        expect(await wiry.blackListed(clientAcc1.address)).to.equal(true);
      });
      ...
      it("Should fail to add zero address to the blacklist", async () => {
        ...
        await expect(wiry.addToBlackList(zeroAddress)).to.be.revertedWith(
          "Wiry: Zero address!"
        );
      });
    ...
    describe("Remove from blacklist", () => {
      ...
      it("Should fail to remove users from the blacklist if caller is not the owner", async () => {
        ...
        await expect(
          wiry.connect(clientAcc1).removeFromBlackList(clientAcc1.address)
        ).to.be.revertedWith("Ownable: caller is not the owner");
      });
      ...
      it("Should fail to remove zero address from the blacklist", async () => {
        ...
        await expect(wiry.removeFromBlackList(zeroAddress)).to.be.revertedWith(
          "Wiry: Zero address!"
        );
      });
    ...
    describe("Forbid token transfers", () => {
      ...
      it("Should forbid sending tokens for blacklisted users", async () => {
        ...
        await expect(
          wiry.connect(clientAcc1).transfer(clientAcc2.address, transferAmount)
